...
const make = Effect.fn("desktop.backendMode.make")(function* (argv: readonly string[]) {
...
/**
 * The effective launch mode for a caller that is not itself about launching. A context without the
 * service is a managed launch, which is what every path outside `DesktopApp` bootstrap assumed
 * before this fork added a second mode, so an upstream call site keeps its own behavior.
 */
export const effectiveModeOrManaged = Effect.gen(function* () {
  const service = yield* Effect.serviceOption(DesktopBackendMode);
  if (Option.isNone(service)) return "managed" as const;
  return (yield* service.value.get).effectiveMode;
});
