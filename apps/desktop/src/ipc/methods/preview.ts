...
const previewForSender = Effect.fn("desktop.ipc.preview.resolveSender")(function* (
  ...
  // Upstream's invoke event carries only the sender's WebContents id, so the
  // sending window is resolved from that id rather than widening the shared
  // IPC type; every fork-owned routing rule stays inside this module.
  const senderContents = event === undefined ? undefined : webContents.fromId(event.sender.id);
  if (senderContents === undefined) {
