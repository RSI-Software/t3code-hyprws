...
export const make = Effect.gen(function* PreviewManagerMake() {
  ...
  const createEntry = Effect.fn("PreviewManager.createWindowOperations")(function* (
    ...
    return { identity, operations, scope, windowClosed: false } satisfies WindowOperationsEntry;
  });
  ...
  const authorizeTab = Effect.fn("PreviewManager.authorizeTab")(function* (
    ...
    if (entry.windowClosed) return yield* new PreviewTabNotFoundError({ tabId });
  ...
  const setWindow = Effect.fn("PreviewManager.setWindow")(function* (
    ...
    entry.window = window;
    entry.windowClosed = false;
    yield* entry.operations.setMainWindow(window, () => {
      if (entry.window !== window) return;
      entry.windowClosed = true;
      runFork(disposeWindow(identity));
    });
  });
