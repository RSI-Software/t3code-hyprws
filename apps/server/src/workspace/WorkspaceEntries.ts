...
import * as WorkspaceEntriesFork from "./WorkspaceEntries.fork.ts"; // fork-hook: workspace-files/workspace-entries-ignored-listing-import
import * as VcsDriverRegistry from "../vcs/VcsDriverRegistry.ts"; // fork-hook: workspace-files/workspace-entries-ignored-registry-import
...
export const make = Effect.gen(function* () {
  ...
  const vcsDrivers = yield* VcsDriverRegistry.VcsDriverRegistry; // fork-hook: workspace-files/workspace-entries-ignored-registry
  ...
  const list: WorkspaceEntries["Service"]["list"] = Effect.fn("WorkspaceEntries.list")(
    function* (input) {
      ...
      if (input.directoryPath !== undefined) {
        ...
        // fork-hook: workspace-files/workspace-entries-directory-ignored-filter — the
        // fork's show-ignored preference filters the directory listing when the client
        // explicitly asks to hide: only an explicit `includeIgnored: false` hides, so the
        // upstream default (undefined) keeps listing ignored children, flagged.
        return {
          entries: entries.flatMap((entry) => {
            if (!ignored.has(entry.path)) return [entry];
            if (input.includeIgnored === false) return [];
