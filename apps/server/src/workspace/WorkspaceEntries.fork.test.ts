...
it.layer(TestLayer, { excludeTestServices: true })("WorkspaceEntries", (it) => {
  ...
  describe("list", () => {
    ...
    it.effect("filters ignored children from directory listings only when requested", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ git: true });
        ...
        yield* writeTextFile(cwd, "ignored.txt", "ignored\n");
        const workspaceEntries = yield* WorkspaceEntries.WorkspaceEntries;
        // An explicit false hides ignored children; undefined keeps upstream's
        // list-everything default, with ignored children still flagged.
        const hidden = yield* workspaceEntries.list({
          cwd,
          directoryPath: "",
          includeIgnored: false,
        });
        expect(hidden.entries.map((entry) => entry.path)).toEqual([".gitignore", "src"]);
        const flagged = yield* workspaceEntries.list({ cwd, directoryPath: "" });
        ...
        const shown = yield* workspaceEntries.list({
          cwd,
          directoryPath: "",
          includeIgnored: true,
        });
