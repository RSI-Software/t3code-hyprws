    ...
    <article className="grid grid-cols-[1rem_minmax(0,1fr)] gap-x-2 border-b border-border/40 px-3 py-2.5 last:border-b-0">
      ...
      <div className="min-w-0">
        <div className="flex min-w-0 items-baseline gap-2">
          ...
          <time className="ml-auto shrink-0 font-mono text-3xs text-muted-foreground/70">
...
function StructuredActivityData({ entry }: { entry: SubagentDetailEntry }) {
  ...
  return (
    <details className="mt-1.5 text-xs" onToggle={(event) => setOpen(event.currentTarget.open)}>
      ...
      {structured ? (
        <pre className="mt-1 max-h-64 overflow-auto whitespace-pre-wrap break-words rounded-md bg-muted/35 p-2 font-mono text-2xs leading-relaxed text-foreground/80">
          {structured}
        </pre>
      ) : null}
    </details>
    ...
    >
      ...
      <header className="border-b border-border/60 px-2 py-2">
        <div className="flex items-center gap-2">
          ...
          <span className="ml-auto rounded-sm border border-border/60 px-1.5 py-0.5 text-3xs text-muted-foreground">
            Read only
          </span>
        </div>
      </header>
