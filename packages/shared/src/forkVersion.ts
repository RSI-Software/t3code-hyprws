...
/** Exact `major.minor.patch` SemVer only: never a dist-tag or a range. The
    web client passes this the exact version string off the wire, and matching
    loosely could send an upstream client at a fork-only release or a fork
    client at the npm registry. */
 ...
 * so the version-skew rule below and the web client (the manual update
 * command) agree on one predicate.
 */
