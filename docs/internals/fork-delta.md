...
| feat(web): add rich Markdown editing (#28)          | markdown-editing | At `v0.0.37-nightly.20260829.1217`, the `pnpm-lock.yaml` conflict came from the unrelated Messages Glass Lab removal and release-version updates. The target ships no rich Markdown editor, safe frontmatter round trips, or Rich/Source surface, so there is no upstream replacement and the fork behavior stays. | v0.0.37-nightly.20260829.1217 |
