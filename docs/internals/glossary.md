...
| `fork-churn`            | The ledger of conflict churn across landed unblock walks, kept as `fork-churn.json` on the bot-owned `refs/fork/churn` so no fork commit carries it. Each apply appends a row and the blocked report assesses it into lesson guidance; no section is posted. |
