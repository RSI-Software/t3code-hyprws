...
| `proofs`                                  | Unique `{name, artifact, sha256}` references, including exactly one each of `snapshot-tests`, `composition`, `test-ownership`, and `compatibility`. `artifact` is relative to the external manifest. Build and every later gate require a regular external JSON file, re-hash its bytes, and require `schema: "fork.rewrite-proof.v1"`, the matching name and source, and `verdict: "pass"`. The rest of each reviewed proof remains input evidence, not tests run by the constructor.                                                                             |
| `expected`                                | Reviewed nonnegative counts for `changedSlots`, `unchangedSlots`, and `removedSignatures`. Changed and unchanged slots must account for the complete stack; construction derives changed slots from original/result tree inequality and recomputes all three before writing objects.                                                                                                                                                                                                                                                                               |
| `unresolved`                              | An empty array is required; unfinished source/test/compatibility proof is not an executable manifest.                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `overrides`                               | Present only when an operator chose part of the fold: `attributed` `{path, commit}` decisions that placed changes, `left` paths a reshape diff carried and the fold skipped, and `unused` flag text for overrides that bound to nothing. The reader binds each half to the slots — an attribution owns every change at its path and starts no later than the first slot folding it, an excluded path owns none — so neither copy can be edited alone. Absent means the manifest predates the record or nothing was hand-chosen; then no change may carry `origin`. |
...
| `changes`                                 | `{path, before, after, reason}` using the same entry shape, plus `origin` (`{kind: "blame"}` or `{kind: "operator", commit}`) on every change when `overrides` is present and on none when it is absent. Compose overlapping transforms into one reviewed change per path; duplicate paths refuse. Replacement objects must already exist.                                                                                                                                                                                                                         |

An `overrides` record is reviewed evidence, not a derivation. `fold-reshape --attribute` and
`--leave` are the only parts of a fold nothing derives, and a rewrite stays tree-neutral whichever
slot absorbs a change, so no proof catches a wrong attribution and only a reviewer reading the
record can. Enumerate the decisions before signing off:

```bash
jq -r '.overrides | (.attributed[] | "attribute \(.path)=\(.commit)"), (.left[] | "leave \(.)"), (.unused[] | "unused \(.)")' manifest.json
```

`unused` carries every flag that had no effect, as the operator typed it. A flag that did nothing
changes nothing the fold derives, so a run whose overrides were all unused still succeeds; it just
says so. Read the record's presence as "a human passed flags here", not as "a human moved a change".
...
cannot bypass it after construction. Proof artifacts use the same regular external-file boundary;
changing, removing or redirecting one invalidates construction and every later gate.
