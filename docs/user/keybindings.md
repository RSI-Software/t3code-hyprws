...
`project.openWindow` opens or reveals the active project's desktop window and defaults to `mod+alt+o`.
The project-window command and its keybinding are shown only in the desktop app.
Repeating either search shortcut closes that search, and switching shortcuts replaces the open search.
