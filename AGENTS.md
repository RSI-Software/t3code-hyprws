...
- Run `vp run fork:ci` before every pull request. It derives the CI scan flags and runs the rebase scan plus the whole scripts workspace — the same checks the fork's pull-request CI runs — so scripts tests are never hand-picked.
