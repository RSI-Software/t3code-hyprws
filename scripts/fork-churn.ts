...
export const renderMarkdown = (
  ...
  // A stopped walk is a walk, so every walk is listed; the split below and the walks-table row
  // say which kind each one is (RSI-Software/t3code-hyprws#1023).
  const entries = rawEntries.filter((entry) => entry.pending !== true);
  const stopped = rawEntries.length - entries.length;
  const lines: Array<string> = [
    "# Fork conflict churn",
    "",
    "> Generated by `vp run fork:churn`. Do not edit by hand.",
    "",
    // Stopped walks are listed (walks table) and their seams counted (hot seams); the tag range
    // states what the fork has actually landed on, so it covers applied walks only.
    "> Stopped walks are listed and their seams counted; the tag range covers applied walks only.",
    "",
    stopped === 0
      ? `- Entries: ${rawEntries.length}`
      : `- Entries: ${rawEntries.length} (${entries.length} applied, ${stopped} stopped)`,
    // A stopped walk attempted a tag; it did not reach it, so the range stays applied-only.
    `- Tag range (applied walks only): ${entries.length === 0 ? "none" : `${code(entries[0]?.tag ?? "")} → ${code(entries.at(-1)?.tag ?? "")}`}`,
    "",
    "## Hot seams",
    "",
  ];
  const hot = hotSeams(rawEntries);
  if (hot.length === 0) {
    lines.push("None.", "");
  } else {
  ...
  // The per-file, per-commit, per-domain and KPI sections describe the landed stack, so they
  // read applied walks only; the walks table carries the stopped walks' rows in full.
  lines.push("## Per file", "");
  ...
  lines.push("## Walks", "");
  // Every walk is listed; a stopped walk renders its stop in the Range cell instead of a head.
  if (rawEntries.length === 0) {
    ...
    for (const entry of rawEntries) {
      ...
      // A stopped walk has no `after`: it attempted the tag but did not reach it, so the
      // outcome is named in the Range cell instead of a head (RSI-Software/t3code-hyprws#1023).
      const rangeCell =
        entry.pending === true
          ? `${code(entry.before)} → **stopped**`
          : `${code(entry.before)} → ${code(entry.after)}`;
      lines.push(
        `| ${code(entry.tag)} | ${rangeCell} | ${elapsedCell} | ${effortCell} | ${repairsCell} | ${classHistogram(entry.conflicts) || "none"} | ${agent}/${human} | [record](${entry.recordUrl}) |`,
      );
    }
    lines.push("");
  }
