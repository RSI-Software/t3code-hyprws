...
const hookReapply = (
  ...
  if (!forkSideIsHookOnly(stages))
    return hookDeclined(
      path,
      `fork-hook reapply of ${keys} refused: the fork side of this seam adds lines beyond its marked hooks, or removes a base line, so there is no mechanical seam to lift`,
      keepBothReason,
    );
